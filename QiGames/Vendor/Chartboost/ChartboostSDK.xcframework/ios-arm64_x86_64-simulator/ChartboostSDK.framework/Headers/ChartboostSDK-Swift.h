...
- (nonnull instancetype)initWithHtml:(NSString * _Nonnull)html baseURL:(NSURL * _Nullable)baseURL webViewDelegate:(id <CHBWebViewDelegate> _Nullable)webViewDelegate suppressIncrementalRendering:(BOOL)suppressIncrementalRendering OBJC_DESIGNATED_INITIALIZER;
...
/// Opens <code>url</code>. When <code>universalLinksToInstalledAppsOnly</code> is <code>true</code>, the URL is opened only if it
/// is a valid universal link to an installed app; otherwise the completion handler is called
/// with <code>false</code> and nothing is opened.
- (void)openURL:(NSURL * _Nonnull)url universalLinksToInstalledAppsOnly:(BOOL)universalLinksToInstalledAppsOnly completionHandler:(void (^ _Nonnull)(BOOL))completionHandler;
...
/// Network request failed. Invalid network request.
  CHBErrorCodeConnectivityInvalidRequest = 206,
/// An unknown error has occurred.
...
/// Progressive video buffering has failed before the required duration was buffered.
  CHBErrorCodeLoadProgressiveBufferingFailed = 323,
/// An unknown error has occurred.
...
/// Invalid or unrecognized clickthrough URL.
...
/// SKProductViewController failed to load the StoreKit product.
  CHBErrorCodeRenderLoadSKProductFailed = 508,
/// Click ignored due to no matching user gesture recognized.
  CHBErrorCodeRenderClickIgnoredNoGesture = 509,
/// Click ignored due to SDK busy handling another click.
  CHBErrorCodeRenderClickIgnoredBusy = 510,
/// The ad was dismissed unexpectedly (programmatically by the publisher/mediator).
  CHBErrorCodeRenderUnexpectedDismiss = 511,
/// An unknown error has occurred.
...
/// LGPD (Brazil) behavioral-targeting consent, or <code>nil</code> when the publisher hasn’t set it.
@property (nonatomic, readonly, strong) NSNumber * _Nullable lgpd;
...
@class OMIDChartboostVerificationScriptResource;
/// <em>WARNING: DO NOT USE. THIS IS AN INTERNAL API AND IT MIGHT CHANGE AT ANY POINT WITHOUT PREVIOUS NOTICE.</em>
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSArray<OMIDChartboostVerificationScriptResource *> * _Nonnull omVerificationResources;)
+ (NSArray<OMIDChartboostVerificationScriptResource *> * _Nonnull)omVerificationResources SWIFT_WARN_UNUSED_RESULT;
@end
...
/// The non-tracking endpoints for New Rendering Pipeline.
...
/// The tracking endpoints for New Rendering Pipeline.
/// This is a mapping of the ad format type (e.g. <code>banner</code>, <code>interstitial</code>, etc) to the URL
/// string for tracking endpoints. These endpoints should only be used when tracking is
/// authorized (ATT status is authorized OR iOS version < 17).
/// If no tracking endpoint is provided for a format, the non-tracking endpoint should be used.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSDictionary<NSString *, NSString *> * _Nullable newRenderingPipelineWaterfallTrackingEndpoints;)
+ (NSDictionary<NSString *, NSString *> * _Nullable)newRenderingPipelineWaterfallTrackingEndpoints SWIFT_WARN_UNUSED_RESULT;
@end
...
@protocol CHBSessionEventTrackersConfiguration
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSArray<NSDictionary<NSString *, id> *> * _Nonnull objCSessionEventTrackers;)
+ (NSArray<NSDictionary<NSString *, id> *> * _Nonnull)objCSessionEventTrackers SWIFT_WARN_UNUSED_RESULT;
@end
